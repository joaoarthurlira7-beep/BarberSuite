    ...
    <div className="min-h-screen bg-[#050505] selection:bg-[#d4af37] selection:text-black">
      ...
      <header className="relative pt-32 pb-20 md:pt-48 md:pb-32 px-4 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center animate-fade-in-up">
          ...
          <h1 className="font-[family-name:var(--font-display)] text-5xl md:text-7xl lg:text-8xl text-white uppercase tracking-tighter leading-none mb-6">
            {barbershop.tagline
              ? <>{barbershop.tagline.split('.')[0]}<br/><span className="text-gold-gradient italic pr-2">{barbershop.tagline.split('.').slice(1).join('.')}</span>.</>
              : barbershop.name
            }
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <BookingModalClientWrapper 
              ...
              barbers={barbers ?? []} 
              services={services ?? []} 
              className="btn-gold py-3 px-8 text-sm w-full sm:w-auto justify-center"
            />
            {barbershop.whatsapp && (
              <a 
      ...
      <section id="servicos" className="py-20 md:py-32 px-4 relative">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="font-[family-name:var(--font-display)] text-3xl md:text-5xl text-white uppercase tracking-tight">Nossos <span className="text-[#d4af37]">Serviços</span></h2>
            ...
            {services.map((service, i) => (
              <div key={service.id} className="group cursor-default animate-fade-in-up" style={{ animationDelay: `${i * 100}ms` }}>
                <div className="flex items-end justify-between mb-2">
                  <h3 className="font-[family-name:var(--font-display)] text-xl text-white tracking-wide group-hover:text-[#d4af37] transition-colors duration-300">
                    {service.name}
                  </h3>
                  <div className="flex-1 border-b border-dashed border-neutral-800 mx-4 mb-1 group-hover:border-[#d4af37]/30 transition-colors" />
                  <span className="text-[#d4af37] font-bold text-lg">{formatCurrency(service.price)}</span>
                </div>
                ...
                <div className="h-px w-0 bg-gradient-to-r from-[#d4af37] to-transparent mt-4 group-hover:w-full transition-all duration-700 ease-out" />
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="equipe" className="py-20 md:py-32 px-4 bg-[#0a0a0a]">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="font-[family-name:var(--font-display)] text-3xl md:text-5xl text-white uppercase tracking-tight">Profissionais de <span className="text-[#d4af37]">Elite</span></h2>
            <div className="w-24 h-1 bg-[#d4af37] mx-auto mt-6" />
          </div>
            ...
            {barbers.map((barber) => (
              <div key={barber.id} className="premium-card overflow-hidden group">
                ...
                <div className="p-8 relative -mt-20 z-10">
                  <span className="text-[10px] text-[#d4af37] uppercase tracking-widest font-bold block mb-1">{barber.role}</span>
      ...
      <section id="avaliacoes" className="py-20 md:py-32 px-4 relative overflow-hidden">
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-16">
            <h2 className="font-[family-name:var(--font-display)] text-3xl md:text-5xl text-white uppercase tracking-tight">O Que Dizem <span className="text-[#d4af37]">Os Clientes</span></h2>
            <div className="w-24 h-1 bg-[#d4af37] mx-auto mt-6" />
          </div>

          <div className="flex gap-6 overflow-x-auto pb-8 snap-x scrollbar-hide px-4 md:px-0">
            {reviews.map((review) => (
              <div key={review.id} className="premium-card p-8 min-w-[300px] md:min-w-[400px] snap-center shrink-0 border-t-4 border-t-[#d4af37]">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-neutral-800 flex items-center justify-center font-bold text-[#d4af37]">
      ...
      <footer className="bg-[#0a0a0a] pt-20 pb-10 px-4 border-t border-neutral-900">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between gap-12 mb-16">
          <div className="max-w-sm">
            ...
            <div className="flex gap-4">
              {barbershop.instagram && (
                <a href={`https://instagram.com/${barbershop.instagram}`} className="w-10 h-10 rounded-full bg-neutral-900 flex items-center justify-center text-neutral-400 hover:text-[#d4af37] hover:bg-[#d4af37]/10 transition-colors">
                  <Instagram size={18} />
                </a>
              )}
              {barbershop.whatsapp && (
                <a href={`https://wa.me/${barbershop.whatsapp}`} className="w-10 h-10 rounded-full bg-neutral-900 flex items-center justify-center text-neutral-400 hover:text-[#d4af37] hover:bg-[#d4af37]/10 transition-colors">
                  <Phone size={18} />
                </a>
              )}
            </div>
          ...
          <div className="flex flex-col gap-4 text-sm text-neutral-400">
            ...
            {barbershop.phone && <span className="flex items-center gap-3"><Phone size={16} className="text-[#d4af37]" /> {barbershop.phone}</span>}
            {barbershop.address && <span className="flex items-start gap-3 max-w-[250px]"><MapPin size={16} className="text-[#d4af37] flex-shrink-0 mt-1" /> {barbershop.address}</span>}
          </div>
        </div>
