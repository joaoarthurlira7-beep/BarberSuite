    ...
    <div className="p-6 max-w-5xl mx-auto w-full">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
        ...
        <button 
          onClick={handleSave} 
          disabled={isSaving}
          className="btn-gold py-2 text-xs w-full md:w-auto"
        >
        ...
        <div className="w-full lg:w-64 flex-shrink-0">
          <nav className="flex lg:flex-col overflow-x-auto lg:overflow-visible gap-2 pb-2 lg:pb-0">
            <button 
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all whitespace-nowrap ${activeTab === 'geral' ? 'bg-[#d4af37]/10 text-[#d4af37]' : 'text-neutral-400 hover:text-white hover:bg-neutral-800/50'}`}
            >
              <Store size={18} /> <span className="text-sm font-medium">Dados Gerais</span>
            </button>
            <button 
              onClick={() => setActiveTab('aparencia')}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all whitespace-nowrap ${activeTab === 'aparencia' ? 'bg-[#d4af37]/10 text-[#d4af37]' : 'text-neutral-400 hover:text-white hover:bg-neutral-800/50'}`}
            >
              <LinkIcon size={18} /> <span className="text-sm font-medium">Página Pública</span>
            </button>
            <button 
              onClick={() => setActiveTab('horarios')}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all whitespace-nowrap ${activeTab === 'horarios' ? 'bg-[#d4af37]/10 text-[#d4af37]' : 'text-neutral-400 hover:text-white hover:bg-neutral-800/50'}`}
            >
              <Calendar size={18} /> <span className="text-sm font-medium">Horários</span>
            </button>
            <button 
              onClick={() => setActiveTab('notificacoes')}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all whitespace-nowrap ${activeTab === 'notificacoes' ? 'bg-[#d4af37]/10 text-[#d4af37]' : 'text-neutral-400 hover:text-white hover:bg-neutral-800/50'}`}
            >
          ...
          {activeTab === 'aparencia' && (
            <div className="space-y-8 animate-fade-in-up">
              ...
              <div>
                ...
                <div className="flex items-center gap-6">
                  <div className="w-24 h-24 rounded-full bg-neutral-900 border border-neutral-800 flex items-center justify-center overflow-hidden">
                    <span className="text-[#d4af37] text-2xl font-bold">V</span>
                  </div>
                  <div>
              ...
              <div>
                ...
                <div className="w-full h-32 rounded-xl bg-neutral-900 border border-neutral-800 flex items-center justify-center flex-col cursor-pointer hover:border-[#d4af37]/50 transition-colors">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="flex gap-4 items-center">
                    <div className="w-10 h-10 rounded-lg bg-[#d4af37] border border-neutral-700"></div>
                    <input type="text" className="premium-input flex-1 font-mono text-sm" defaultValue="#d4af37" />
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6 animate-fade-in-up">
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-white flex items-center gap-2">WhatsApp Lembrete <span className="bg-[#d4af37]/20 text-[#d4af37] text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-widest">PRO</span></h3>
